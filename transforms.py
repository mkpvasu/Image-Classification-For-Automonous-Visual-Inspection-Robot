...
class RGBTransform(object):
    ...
    def _then(self, operation):
        return RGBTransform(np.dot(embedIMat(operation), self._matrix))

    def tintWith(self, base_color, factor=1.0):
        ...
        """
        base_color = toRGB(base_color, "base_color")
        operation = embedIMat((1 - factor) * np.eye(3))
    ...
    def applyToImg(self, image):
        ...
        matrix = tuple(self.getMatrix().flatten())
