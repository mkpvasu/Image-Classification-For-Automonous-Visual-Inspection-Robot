...
class ModelPredict:
    def __init__(self, partition_images_path, batch_size=64):
        super().__init__()
        self.partitionImagesPath = partition_images_path
    ...
    def predictImages(self, current_model):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        predictionDataDf, predictionLoader = self.prepareData()
        predictionResults = []
        currentModel = current_model.load_state_dict(torch.load("*_best-weights.pth"))
